...
void loop() {
    ...
    if (inCalibrationMode) {
            ...
            for (int r = 0; r < rows; r++) {
                ...
                if (currentButtonState != buttonState[r][c]) {
                    ...
                    if (currentButtonState) {
                        isAnyButtonPressed = true; // Mark that a button is pressed
        ...
        // Check High C button state
        bool highCButtonState = !digitalRead(highCButton);
        if (highCButtonState != lastHighCButtonState) {
            lastHighCButtonState = highCButtonState; // Update last button state for the next loop iteration

            if (highCButtonState) {
                int noteIndex = 48; // High C note index
                switch(octaveShift) {
        ...
        isAnyButtonPressed = isAnyButtonPressed || highCButtonState;
